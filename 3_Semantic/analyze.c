...
static void insertNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		// Parameters
		case Params: //Filled Skeleton Code
		{
			if(TEST) printf("params\n");
		...
		// Variable Access
		case VarAccessExpr: //Filled Skeleton Code
		{
			if(TEST) printf("var access\n");
			// Semantic Error: Undeclared Variables
			SymbolRec *symbol = lookupSymbolWithKind(currentScope, t->name, VariableSym);
			if (symbol == NULL) UndeclaredVariableError(currentScope, t);
			// Update Symbol Table Entry
			else
...
static void checkNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case IfStmt:
		case WhileStmt: //Filled Skeleton Code
		{
			if(TEST) printf("if while\n");
			// Error Check
		...
		// Return Statement
		case ReturnStmt: //Filled Skeleton Code
		{
			if(TEST) printf("return\n");
			// Error Check
			ERROR_CHECK(currentScope->func != NULL);
			// Semantic Error: Invalid Return
			// Return variable in Void function
			if(t->flag == TRUE && currentScope->func->type != Void)
		...
		case AssignExpr:
		case BinOpExpr: //Filled Skeleton Code
		{
			// Error Check
		...
		// Call Expression
		case CallExpr: //Filled Skeleton Code
		{
			...
			if(!(paramNode == NULL || paramNode->type == None || paramNode->type == Void) || !(argNode == NULL || argNode->type == Void))
		...
		case VarAccessExpr:
		{
			if(TEST) printf("var access expr\n");
			SymbolRec *symbol = lookupSymbolWithKind(currentScope, t->name, VariableSym);
			// Error Check
			...
			// Array Access or Not
			if (t->child[0] != NULL) //Filled Skeleton Code
			{
